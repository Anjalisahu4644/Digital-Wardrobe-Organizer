...
const App = {
  ...
  bindAuthEvents() {
    ...
    document.getElementById('register-form').addEventListener('submit', async (e) => {
      ...
      if (!name || !username || !password) { errEl.textContent = 'Fill in all fields'; return; }
  ...
  async navigate(page) {
    ...
    document.querySelectorAll('.mob-link').forEach(l => l.classList.toggle('active', l.dataset.page === page));
  ...
  async bindDashboardEvents() {
    ...
    document.querySelectorAll('.dash-item').forEach(el => {
      el.addEventListener('click', () => Wardrobe.showItemDetail(el.dataset.id));
    });
    ...
    if (weatherEl) {
      try {
        ...
        const spinnerEl = weatherEl.querySelector('.spinner');
        if (spinnerEl) spinnerEl.outerHTML = '<p style="color:var(--text-muted);font-size:0.8rem;">Weather unavailable</p>';
      }
    }
    ...
    if (quickChart) {
      ...
      if (entries.length === 0) {
        quickChart.innerHTML = '<p style="color:var(--text-muted);font-size:0.85rem;">Add items to see category breakdown</p>';
      } else {
        const max = Math.max(...entries.map(e => e[1]), 1);
        quickChart.innerHTML = entries.map(([label, count]) => `
  ...
  // ---- Wardrobe Events ----
  bindWardrobeEvents() {
    document.getElementById('add-item-btn').addEventListener('click', () => Wardrobe.showAddModal());
  ...
  // ---- Outfit Events ----
  bindOutfitEvents() {
    document.getElementById('create-outfit-btn').addEventListener('click', () => Outfits.showBuilder());
  },
